...
int main(int argc, char* argv[])
{
    ...
    auto marker_pub = node->create_publisher<visualization_msgs::msg::MarkerArray>(
        "/circle_markers", rclcpp::QoS(1).transient_local());
    ...
    RCLCPP_INFO(logger, "Step 2: planning to lowered start pose...");
    {
        std::map<std::string, double> joint_targets = {
            ...
            {"joint_2", 20.0 * M_PI / 180.0},
            {"joint_3",  0.0 * M_PI / 180.0},
            {"joint_4", -70.0 * M_PI / 180.0},
    ...
    // ── Step 3: move joint-space to the circle start point ──────────────────
    RCLCPP_INFO(logger, "Step 3: moving to circle start point...");
    move_group.setMaxVelocityScalingFactor(0.3);
    move_group.setMaxAccelerationScalingFactor(0.3);

    auto eef_pose = move_group.getCurrentPose("ee").pose;

    // Center the circle at current EEF position (ZY plane, X fixed)
    const double cx = eef_pose.position.x + 0.02;  // keep arm extended forward
    const double cy = eef_pose.position.y ;  // shift circle center slightly to the left
    const double cz = eef_pose.position.z ;  // shift circle center slightly up to avoid table collisions
    const double R  = 0.05;  // radius: 8 cm — safe for most arm poses
    const int    N  = 40;    // waypoints (every 10°)

    // Build circle in ZY plane (z = up/down, y = left/right)
    ...
    for (int i = 0; i <= N; ++i)
    {
        ...
        p.position.x = cx;
        p.position.y = cy + R * std::cos(angle);
        p.position.z = cz + R * std::sin(angle);
        waypoints.push_back(p);
    }

    // ── Step 3: Cartesian move to circle start point ─────────────────────────
    RCLCPP_INFO(logger, "Step 3: Cartesian move to circle start point...");
    {
        ...
        if (frac < 0.5)
        {
            ...
            if (!ok)
            {
                RCLCPP_ERROR(logger, "Failed to reach circle start point.");
                return false;
            }
            publish_display(fallback.trajectory_, "to_wp1");
            rclcpp::sleep_for(std::chrono::seconds(2));
            if (!check_ok()) return false;
        ...
        else
        {
            ...
            publish_display(approach_traj, "to_wp1");
            rclcpp::sleep_for(std::chrono::seconds(2));
            if (!check_ok()) return false;
    ...
    RCLCPP_INFO(logger, "Reached circle start point.");

    // ── Step 4: Cartesian circle ──────────────────────────────────────────────
    RCLCPP_INFO(logger, "Step 4: computing Cartesian circle (r=%.2f m, %d pts)...", R, N);

    // Publish planned circle path as LINE_STRIP + center SPHERE in RViz
    {
        visualization_msgs::msg::MarkerArray ma;

        // LINE_STRIP — planned circle
        visualization_msgs::msg::Marker line;
        line.header.frame_id = "base_link";
        line.header.stamp    = node->now();
        line.ns = "circle"; line.id = 0;
        ...
        RCLCPP_INFO(logger, "Published planned circle markers.");
    }
    ...
    if (fraction < 0.5)
    {
        RCLCPP_ERROR(logger, "Cartesian circle too short (%.1f%%). Aborting.", fraction * 100.0);
        return false;
    }

    RCLCPP_INFO(logger, "Cartesian circle %.1f%% — displaying...", fraction * 100.0);
    ...
    if (!check_ok()) return false;
    RCLCPP_INFO(logger, "Cartesian execution complete.");
