from arm_bot_hw.src import usb_class, can_value_type
...
class Motor_Control:
    ...
    def control_pos_force(self, DM_Motor, pos: float, vel: float, torque: float):
        id_ = DM_Motor.GetCanId()
        if id_ not in self.motors:
            print(f"[Error] In control_pos_force, no motor with id {id_} is registered.", file=sys.stderr)
            sys.exit(-1)

        can_id = id_ + Control_Mode.POS_FORCE_MODE

        m = self.motors[id_]
        lp = m.get_limit_param()
        vel_u16    = int((vel    / lp[1]) * 65535)
        torque_u16 = int((torque / lp[2]) * 65535)

        pbuf = struct.pack('<f', pos)
        vbuf = struct.pack('<H', vel_u16)     # uint16_t, little-endian
        ibuf = struct.pack('<H', torque_u16)  # uint16_t, little-endian
        mydata = list(pbuf + vbuf + ibuf)
